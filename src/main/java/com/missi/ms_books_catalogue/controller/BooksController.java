package com.missi.ms_books_catalogue.controller;
...
import com.missi.ms_books_catalogue.service.BooksService;
...
public class BooksController {
    ...
    public ResponseEntity<List<Book>> getBooks(
            ...
            @Parameter(name = "title", description = "Título del libro. No tiene por que ser exacto", example = "El Quijote", required = false)
            @RequestParam(required = false) String title,
            @Parameter(name = "author", description = "Autor del libro. Debe ser exacto", example = "Cervantes", required = false)
            @RequestParam(required = false) String author,
            @Parameter(name = "publicDate", description = "Fecha de publicación del libro. No tiene por que ser exacta", example = "1605", required = false)
            ...
            @Parameter(name = "category", description = "Categoría del libro. Debe ser exacta", example = "Novela", required = false)
            @RequestParam(required = false) String category,
            @Parameter(name = "isbn", description = "ISBN del libro. Debe ser exacto", example = "978-3-16-148410-0", required = false)
            @RequestParam(required = false) String isbn,
            @Parameter(name = "score", description = "Puntuación del libro. Debe ser exacta", example = "4.5", required = false)
            @RequestParam(required = false) Double score,
            @Parameter(name = "price", description = "Precio del libro. Debe ser exacto", example = "19.99", required = false)
            @RequestParam(required = false) Double price,
            @Parameter(name = "stock", description = "Stock del libro. Debe ser exacto", example = "100", required = false)
            @RequestParam(required = false) Integer stock) {

        log.info("headers: {}", headers);
        List<Book> books = service.getBooks(title, author, publicDate, category, isbn, score, visible, price, stock);
        if (books != null) {
    ...
    public ResponseEntity<Book> patchBook(@PathVariable String bookId, @RequestBody String patchBody) {
        ...
        Book patched = service.patchBook(bookId, patchBody);
        if (patched != null) {
